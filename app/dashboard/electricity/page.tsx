...
const DISCO_PROVIDERS = [
  ...
  { id: 'jos-electric', label: 'Jos Electric (JEDC)' },
  { id: 'kaduna-electric', label: 'Kaduna Electric (KAEDCO)' },
  { id: 'kano-electric', label: 'Kano Electric (KEDCO)' },
  { id: 'port-harcourt-electric', label: 'Port Harcourt (PHED)' },
];
...
export default function ElectricityPage() {
  ...
  const handleVerifyMeter = async () => {
    ...
    try {
      const res = await fetch('/api/vtu/verify-meter', {
        method: 'POST',
      ...
      if (res.ok && data.success) {
        ...
        setErrorMsg(data.message || 'Meter verification failed');
      }
    } catch (err) {
      setErrorMsg('Failed to verify meter number');
    } finally {
      setVerifying(false);
    }
  };
  ...
  const handlePurchase = async (e: React.FormEvent) => {
    ...
    if (walletBalance < numericAmount) {
      setErrorMsg(`Insufficient wallet balance. You need ₦${numericAmount} but have ₦${walletBalance}.`);
      return;
    }
    ...
    try {
      ...
      const res = await fetch('/api/vtu/buy-electricity', {
        method: 'POST',
        headers: { 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <h3 className="text-lg font-bold text-gray-900 mb-6 flex items-center gap-2">
              <span>💡</span> Pay Electricity Bill
            </h3>

            {purchasedToken && (
              <div className="mb-6 bg-green-50 border border-green-200 p-4 rounded-xl">
                <p className="text-xs text-green-800 font-semibold mb-1">Electricity Token Generated:</p>
                <p className="text-2xl font-mono font-bold text-green-700 tracking-wider select-all">
                  {purchasedToken}
                </p>
              </div>
            )}
            ...
            <form onSubmit={handlePurchase} className="space-y-6">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">Electricity Provider (Disco)</label>
                <select
                  required
                  ...
                  className="w-full px-4 py-3 rounded-lg border border-gray-300 text-gray-900 focus:ring-2 focus:ring-blue-600 focus:border-transparent outline-none transition text-sm bg-white"
                >
                ...
                <div className="grid grid-cols-2 gap-3">
                  {['prepaid', 'postpaid'].map((type) => (
                    <button
                      ...
                      className={`py-3 px-2 text-center rounded-xl font-bold capitalize border transition text-sm ${
                        meterType === type
