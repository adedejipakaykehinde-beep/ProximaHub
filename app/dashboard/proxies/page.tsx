...
export default function ProxiesPage() {
  ...
  // Prices per proxy unit
  const prices: Record<string, number> = {
    datacenter: 1200, // ₦1,200 per datacenter proxy
    residential: 2500, // ₦2,500 per residential proxy
  };

  const totalPrice = (prices[proxyType] || 1200) * quantity;
  ...
  const handlePurchase = async (e: React.FormEvent) => {
    ...
    if (walletBalance < totalPrice) {
      setErrorMsg(`Insufficient wallet balance. Total cost is ₦${totalPrice.toLocaleString()} but you have ₦${walletBalance.toLocaleString()}.`);
      return;
    }
    ...
    try {
      ...
      const res = await fetch('/api/vtu/buy-proxy', {
        method: 'POST',
        headers: { 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <h3 className="text-lg font-bold text-gray-900 mb-6 flex items-center gap-2">
              <span>🌐</span> Buy High-Speed Proxies
            </h3>
            ...
            <form onSubmit={handlePurchase} className="space-y-6">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Proxy Type</label>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    <p className="font-bold text-gray-900 text-sm">Datacenter Proxy</p>
                    <p className="text-xs text-gray-500 mt-1">High speed & uptime</p>
                    <p className="text-blue-600 font-bold text-xs mt-2">₦1,200 / proxy</p>
                  </button>
                  ...
                  >
                    <p className="font-bold text-gray-900 text-sm">Residential Proxy</p>
                    <p className="text-xs text-gray-500 mt-1">Real IP addresses</p>
                    <p className="text-blue-600 font-bold text-xs mt-2">₦2,500 / proxy</p>
                  </button>
                </div>
              </div>
              ...
              <div className="p-4 bg-gray-50 rounded-xl flex justify-between items-center border border-gray-100">
                ...
                <span className="text-lg font-extrabold text-gray-900">₦{totalPrice.toLocaleString()}</span>
              </div>
              ...
              >
                {loading ? 'Processing...' : 'Purchase Proxies'}
              </button>
            </form>

            {generatedProxies.length > 0 && (
              <div className="mt-8 p-4 bg-blue-50 border border-blue-200 rounded-xl">
                <h4 className="font-bold text-blue-900 text-sm mb-2">Your Proxy List (IP:Port:User:Pass):</h4>
                <div className="bg-white p-3 rounded-lg border border-blue-100 text-xs font-mono text-gray-800 space-y-1 overflow-x-auto">
                  {generatedProxies.map((proxy, idx) => (
                    <div key={idx}>{proxy}</div>
                  ))}
                </div>
              </div>
            )}
