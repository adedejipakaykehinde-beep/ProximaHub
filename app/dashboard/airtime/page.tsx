...
export default function AirtimePage() {
  ...
  const handlePurchase = async (e: React.FormEvent) => {
    ...
    // Phone number format validation (11-digit Nigerian format)
    const cleanPhone = phoneNumber.trim().replace(/\s+/g, '');
    const phoneRegex = /^0(70|80|81|90|91|80|81)\d{8}$/;
    ...
    try {
      ...
      if (res.ok && data.success) {
        alert(`Success! Sent ₦${rechargeAmount} ${selectedNetwork} airtime to ${cleanPhone}`);
        setPhoneNumber('');
        setAmount('');
        fetchUserData();
      } else {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            <form onSubmit={handlePurchase} className="space-y-6">
              ...
              >
                {loading ? 'Processing...' : 'Buy Airtime Now'}
              </button>
            </form>
          </div>
